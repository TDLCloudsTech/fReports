...
class DgiiReport(models.Model):
    ...
    def generate_report(self):
        if self.state == 'generated':
            action = self.env.ref(
                'kl_dgii_reports.dgii_report_regenerate_wizard_action').read()[0]
    ...
    def get_606_tree_view(self):
        return {
            ...
            'view_id':
                self.env.ref('kl_dgii_reports.dgii_report_purchase_line_tree').id,
    ...
    def get_607_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('kl_dgii_reports.dgii_report_sale_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
    ...
    def get_608_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('kl_dgii_reports.dgii_cancel_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
    ...
    def get_609_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('kl_dgii_reports.dgii_exterior_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
